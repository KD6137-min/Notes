...
- 学习率调度器：`optim.lr_scheduler.StepLR`​、`optim.lr_scheduler.LambdaLR`

优化器`.param_groups`属性：优化器中参数组的列表（list of dict），每个参数组都有自己的参数集合、学习率、权重衰减等超参

一个 optimizer 可以同时管理多个“参数组”（param group），每个 group 可以有完全不同的学习率、正则化、动量等
...
**典型用途：**微调预训练模型、冻结部分参数而不冻结另一部分、手动调整某一组学习率、针对不同层使用不同 weight decay



`LambdaLR`：用一个函数（常为lambda，返回一个学习率倍率）来控制学习率的变化

真实学习率计算公式：$lr = initial_{lr} \times lr_{lambda}(epoch)$
