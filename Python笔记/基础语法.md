...
- 生成器：推荐多用
    ...
    - 本质是协程对象`PyGenObject`，内部包含`PyFrameObject,PyCodeObject,locals,instruction pointer`
...
def decorator(func): 	# func即被装饰的函数
    def wrapper(*args, **kwargs):	# 包装函数, 在此添加额外功能
        # 前置逻辑：在调用原函数之前可以执行一些操作
        result = func(*args, **kwargs)
        # 后置逻辑：在调用原函数之后可以执行一些操作
        return result
    return wrapper
```
...
**类装饰器：**通过实现`__call__`方法，装饰器可以是一个类
...
异常链：`raise ... from e`，抛出一个新的 `xxxError`，并把原来的异常e作为原因：
...
`raise xxxError from None`：隐藏原始异常
...
Python **官方内置**的交互式调试器，无需安装，直接 `import pdb`

**功能：**设置断点、单步执行代码、查看变量的值、修改变量、查看当前调用栈、快速定位 bug

**常用主动切入方式：**

```python
# 程序执行到这一行会暂停，然后进入 pdb 交互界面
import pdb; pdb.set_trace()

# 在 Python 3.7+ 中，也可以用更简单的
breakpoint()	# 官方推荐，内部默认调用 pdb
```
...
class WellDocumentedClass:
    ...
    Example:
        >>> obj = WellDocumentedClass("test", 42)			# >>>行 为可以直接在终端中测试用的示例代码
        >>> obj.get_description()
        'test: 42'
    """
...
把文档字符串（docstring）里的示例代码当作测试自动运行
...
doctest 会执行 `add(2, 3)`，检查输出是否等于 `5`，不一致就报错
...
- 在文件末尾加测试入口：
    ...
    运行`python test.py`，没有输出则说明全部通过

- 直接用命令行运行：

    ```python
    python -m doctest test.py -v	# v表berbose详细
    ```
...
def divide(a, b):
    ...
    ...			# 此处...表忽略中间 traceback 内容
    ZeroDivisionError: division by zero
    """
    return a / b
```

底层原理：找到docstring中`>>>`行并用`exec()`执行，捕获stdout或return value转为字符串，和expected_output比较
