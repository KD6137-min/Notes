...
- `.sample(序列, k)`: 指定范围随机选择多个**不重复**元素，k大于序列长度会报错
...
`subprocess.run(command_list)` ：启动并执行一个外部程序（系统命令） ，相当于在 Python 里运行一个 shell / 系统命令

```python
import subprocess

subprocess.run(["ls"])		# 效果等价于在终端执行 ls
...
Abstract Syntax Tree，抽象语法树，把代码解析成一棵树形结构，只保留 **语法结构**，不保留空格、换行、注释
...
安全解析：常用于CLI参数自动类型转换，把字符串解析成 python 数据类型（只允许字符串、数字、list、dict、tuple、bool、None），但不会执行代码，解析失败则保持字符串
...
- 实现最大堆：由于 `heapq`​ 默认是最小堆, 可以通过存储负数模拟最大堆
...
1. **字符串注解**
2. **延迟求值注解**（`from __future__ import annotations`）

你这个场景通常出现在类的方法参数、返回值，或者类属性类型里要写“自己这个类”的时候。
...
这里 `'Node'` 就是**前向引用**。因为在类体还没完全创建好时，直接写 `Node` 可能还解析不了，所以先写成字符串，之后再处理。

如果你不想每次都加引号，可以在文件开头写：

```python
from __future__ import annotations
```

这样类型注解会被**延后解析**，你就可以直接写：
...
- **Python 3.9** 里如果没开这个 future 特性，通常就要写成 `'Node'`
- **Python 3.10+** 也还是经常配合这个 future 用
- 这主要是给**类型注解**用的，不是说普通代码里可以随便提前用未定义对象

比如下面还是不行：

```python
class A:
    x = A()   # 这通常不行，因为类对象 A 这时还没绑定完成
```
...
因为方法体里的 `A()` 是在**运行时调用方法时**才执行，不是在类定义阶段立刻执行。
...
**`from __future__ import annotations` 让注解延迟解析**

你要是愿意，我可以顺手把“为什么类体里有些地方能写自己，有些地方不能写自己”给你彻底捋顺。
