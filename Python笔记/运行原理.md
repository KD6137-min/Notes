...
- 代码开始执行后, 启动Python解释器, 将`.py`文件编译为**字节码对象`PyCodeObject`**(字节码容器，包含源代码中的字符串、常量值、字节码指令及字节码指令和源代码行号的对应关系)
...
- Python虚拟机从`PyCodeObject`对象中依次读入每一条字节码指令, 并在当前的`上下文环境`​中执行这条字节码指令（`PyCodeObject`用于创建并驱动其他对象，其本身一直存在，真正执行的是`PyEval_EvalFrame(code_object)`）
...
由`PyCodeObject`创建一`PyTypeObject`
...
变量存储的是对象的**引用**(在栈中), 实际是一个字符串符号, 用来关联存储在内存中的对象，没有值传递/引用传递，修改只对可变对象有效

Python 里真正重要的分类只有一个：

- 可变对象：`list`、`dict`、`set`、自定义对象，对象内容可以原地修改
- 不可变对象：`int`、`float`、`bool`、`str`、`tuple`、`frozenset`等，对象内容不能被修改，只能重新绑定

**所有赋值，都是“名字 → 对象”的绑定**，是建立变量与对象的映射关系，**引用赋值**, 创建一个新的引用, 即传递原对象内存地址给左侧, 实际上未创建新对象
...
- step1：编译到`def`时，生成一`PyCodeObject`对象，包含字节码、常量表、局部变量名、参数等

- step2：解释器执行`MAKE_FUNCTION`时创建一`PyFunctionObject`对象，注意是执行的时候才创建函数对象，结构包含：
    ...
    - `__closure__`：只捕获自由变量/用到的变量（引用，而非复制），而不是当前环境的所有变量，实际保存的是`cell object(PyCellObject)`，cell对象的`cell_contents`保存变量值，与`__code__.co_freevars`保存的需要从外层捕获的变量名元组一一对应

- step3：绑定变量名

function object被调用时，读取`function.__code__`，创建`frame object(PyFrameObject,执行帧)`由其执行`code object`字节码
...
由`PyCodeObject`生成`PyModuleObject`，执行`exec(code,module.__dict__)`
...
- 静态引入: 最常见, 在代码编写阶段就明确加载某个模块, 可能加载不必要的部分, 影响效率
    ...
    - `from module import *`: 只能导入模块中`__all__`​规定的对外公开的内容, 指定导入则无此限制，不会导入`_`开头的函数
