...
- 安装: 推荐安装**Miniforge**或**Mambaforge**，只包含最核心的`conda`/`mamba`工具和`conda-forge`频道，[官网下载](https://github.com/conda-forge/miniforge)
- 频道 (Channels): 包来源，默认频道往往不够用，特别是科学计算包
    - **强烈建议优先使用`conda-forge`频道：** `conda config --add channels conda-forge` 和 `conda config --set channel_priority strict` (使用`mamba`时自动优化)
...
- 
- 激活环境：
    - **Windows (CMD):** `activate myenv`
    - **Windows (PowerShell):** `conda activate myenv` 或 `mamba activate myenv`
    - **Linux/macOS:** `conda activate myenv` 或 `mamba activate myenv` (有时需先`source <conda安装目录>/bin/activate`初始化shell)
- **停用环境：** `conda deactivate` 或 `mamba deactivate` (在激活的环境中输入)
- **列出所有环境：** `conda env list` 或 `mamba env list`
- **在当前激活的环境中安装包：**
     `conda install numpy pandas` 或 `mamba install numpy pandas`
- **安装特定版本包：** `conda install numpy=1.23`
- **更新当前环境中的某个包：** `conda update numpy` (通常指定包名更新该包及其依赖) 或 `mamba update numpy`
- **更新当前环境中的所有包：** `conda update --all` 或 `mamba update --all` (**谨慎使用，可能导致依赖冲突，特别推荐用`mamba`执行此操作**)
- **移除当前环境中的包：** `conda remove numpy` 或 `mamba remove numpy`
- **导出环境配置到`environment.yml`文件 (包含精确版本和构建号)：** `conda env export > environment.yml`
- **从`environment.yml`文件创建环境：** `conda env create -f environment.yml` 或 `mamba env create -f environment.yml` (如果文件里明确写了`channels`，优先执行此命令)
- **删除整个环境：** `conda env remove -n myenv` 或 `mamba env remove -n myenv`
- **清理缓存（释放空间）：** `conda clean --all` 或 `mamba clean --all`
